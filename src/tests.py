...
from .find_hotspots import main_find_hotspots, find_valid_regions, plot_clusters_with_folium
...
def test_find_hotspots_run(df_total_path, dealer_scope_dict_path, config_file_path, dealer_region_name, product_group_id, date_str_tuple, dbscan_parameters_tuple):
...
def test_show_all_results(df_total_scanning_locations, df_total_centroids, df_result_within_archive, df_suspicious_hotspots_parameters, \
                    dealer_scope_dict_path, start_date_str, end_date_str, dealer_region_name, radius, min_samples):
    ...
    dealer_cluster_cols = ['cluster_label', 'province', 'city', 'district', 'street', 'scanning_count_within_cluster', 'is_remote', 'is_suspicious',
                        'ratio_scanning_count', 'dis_to_all_local_hotspots_centroid', 'std_distance_within_cluster', 
                        'box_count_within_cluster', 'box_count_ratio_for_cluster'
                        ]
    ...
    print('---在档经销商汇总表---')
    print()
    df_suspicious_dealers_to_show = df_result_within_archive.loc[:, suspicious_dealers_overall_cols]
    ...
    for i in range(len(df_result_within_archive)):
        ...
        print(f'---经营范围---')
        print(tabulate(df_busiuness_scope, headers='keys', tablefmt='pretty', showindex=False))
        ...
        for cols in [dealer_cluster_cols]:
            ...
            df_dealer_cluster = df_total_centroids.loc[df_total_centroids.dealer_id == dealer_id, cols]
        ...
        print(tabulate(df_count_merged.loc[df_count_merged['数量']>2, :], headers='keys', tablefmt='pretty', showindex=False))

        m = plot_clusters_with_folium(df_scanning_locations, points_size = 3, noise_size=3, if_save=False)
        display(m)
...
def suspicious_test(df_total_centroids, **kwargs):
    ...
    df_suspicious_hotspots = df_total_centroids_with_suspicious_label.loc[suspicious_mask, :].reset_index(drop=True)

    return len(df_suspicious_hotspots),  std_distance_within_cluster_threshold, df_suspicious_hotspots
...
def show_test_result(df_total_centroids, threshold_name):
    ...
    # 创建折线图
    plt.plot(x, y, marker='o', linestyle='-', color='b')  # 'o' 是圆圈标记，'-' 是折线，'b' 是蓝色
